...
func TestParquetDataSourceWriter(t *testing.T) {
	dataframe := df.NewInmemoryDataframe([]df.Column{{Name: "a", Format: df.IntegerFormat}, {Name: "b", Format: df.DoubleFormat}}, [][]interface{}{
		{int64(1), float64(1.0)},
		{int64(2), float64(2.0)},
		{int64(3), float64(3.0)},
	})
...
func BenchmarkParquetParsing(b *testing.B) {
	...
	dataframe := df.NewInmemoryDataframe([]df.Column{{Name: "a", Format: df.IntegerFormat}, {Name: "b", Format: df.DoubleFormat}, {Name: "c", Format: df.StringFormat}, {Name: "d", Format: df.BoolFormat}}, records)
