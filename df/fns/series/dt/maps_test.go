...
func TestMaskNil(t *testing.T) {
	s1 := inmemory.NewSeries(&[]df.Value{
		inmemory.NewDatetimeValue(time.Date(2009, time.November, 10, 23, 0, 0, 0, time.UTC)),
		inmemory.NewDatetimeValue(time.Date(2010, time.November, 10, 23, 0, 0, 0, time.UTC)),
		inmemory.NewValue(df.DateTimeFormat, nil),
	}, df.DateTimeFormat)
...
func TestParse(t *testing.T) {
	s1 := inmemory.NewStringSeries(&[]string{
		"2022-01-23",
	})
...
func TestFromUnixMilli(t *testing.T) {
	s1 := inmemory.NewIntSeries(&[]int64{
		time.Now().UnixMilli(),
	})
	s2 := FromUnixMilli(s1)
