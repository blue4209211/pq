...
func TestDistinct(t *testing.T) {
	d := inmemory.NewDataframeWithNameFromSeries("df", []string{"c1", "c2"}, &[]df.Series{
		inmemory.NewDoubleSeries(&[]float64{1.0, 2.0, 2.0}),
		inmemory.NewSeries(&[]df.Value{inmemory.NewDoubleValue(1.0), inmemory.NewValue(df.DoubleFormat, 2.0), inmemory.NewValue(df.DoubleFormat, 2.0)}, df.DoubleFormat),
	})
	d2 := Distinct(d)
