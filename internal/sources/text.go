...
func (t *textDataSource) Reader(reader io.Reader, args map[string]string) (DataFrameReader, error) {
	textReader := textDataSourceReader{args: args}
	err := textReader.init(reader)
	return &textReader, err
}

func (t *textDataSource) Writer(data df.DataFrame, args map[string]string) (w DataFrameWriter, err error) {
